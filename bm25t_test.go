...
func TestBM25TGetScores(t *testing.T) {
	...
	// BM25T score = idf * (delta + (qFreq*(1+k))/(qFreq+k))
	scores, err := bm25.GetScores([]string{"hello"})
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	expected := []float64{1.3862943611198906, 0.6931471805599453}
	...
	expected = []float64{1.3862943611198906, 2.7725887222397811}
	for i, score := range scores {
		if !almostEqual(score, expected[i]) {
			t.Errorf("Expected score %.16f at index %d, but got %.16f", expected[i], i, score)
		}
...
func TestBM25TGetBatchScores(t *testing.T) {
	...
	expected = []float64{2.7725887222397811}
	for i, score := range scores {
		if !almostEqual(score, expected[i]) {
			t.Errorf("Expected score %.16f at index %d, but got %.16f", expected[i], i, score)
		}
